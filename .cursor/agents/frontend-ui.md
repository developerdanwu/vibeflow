...
- **You own:** `src/routes/`, `src/components/` (including `ui/` and feature modules like `big-calendar/` when the task is UI), `src/hooks/`, `src/lib/`, `src/integrations/`, `src/data/`.
...
- **We use Base UI, not Radix.** Do **not** use the `asChild` prop—it causes TypeScript errors.
