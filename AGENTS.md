...
VibeFlow is a personal productivity calendar application built with React 19, TanStack Router (SPA), and Convex for real-time data synchronization. The app integrates with Google Calendar via OAuth, enables time blocking for focused work, and provides productivity analytics. Authentication is handled by WorkOS AuthKit.
...
pnpm tauri:dev      # Run Vite dev server and open Tauri window
pnpm tauri:build    # Build static frontend + Tauri app (same vite.config.ts, output in dist/)
```

### Tauri (desktop)
- **Web vs Tauri:** Web and Tauri share the same SPA build and client-side auth (authkit-react). Use `pnpm dev` / `pnpm build` for the web app. Use `pnpm tauri:dev` / `pnpm tauri:build` for the macOS desktop build; Tauri loads the frontend from `dist/`. Redirect URI is `/oauth/workos-callback` (resolved from current origin for web; `http://localhost:3000/oauth/workos-callback` for Tauri dev). Add the appropriate redirect URIs in the WorkOS Dashboard. See [src/docs/tauri.md](src/docs/tauri.md).
- **Tauri API usage:** Any code that calls `@tauri-apps/api` (e.g. `open` from the opener plugin) must run only inside the Tauri webview. Guard with `isTauri()` from `@/lib/tauri` or use dynamic `import("@tauri-apps/plugin-opener")` only when `isTauri()` is true, so the app does not throw in the browser or during SSR. See [src/docs/tauri.md](src/docs/tauri.md).
...
└── Configuration Files
    ...
    ├── vite.config.ts      # Vite configuration (single config for web and Tauri)
...
- **Tauri and OAuth callbacks:** See [src/docs/tauri.md](src/docs/tauri.md) for desktop OAuth (Google deep link, Linear http-only and public callback route).
